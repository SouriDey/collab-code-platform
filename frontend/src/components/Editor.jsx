...
function CodeEditor() {
  ...
  useEffect(() => {
    const loadSavedCode = async () => {
      try {
        const res = await fetch(`https://collab-code-platform-y9nt.onrender.com/api/rooms/${roomId}/code`);
  ...
  const saveCodeToDB = async (newCode, newLanguage) => {
    try {
      ...
      await fetch(`https://collab-code-platform-y9nt.onrender.com/api/rooms/${roomId}/code`, {
        method: "PUT",
        headers: {
